...
def click_named_button(page: Page, captured_values, story_context,
    ...
    # The backend's response is authoritative. An error toast here — e.g.
    # "Relevance lab organization name exist" — means the org was NOT created on
    # THIS run. FAIL right here so the failure screenshot captures THIS page (the
    # error toast on the Create Organization screen), instead of letting later
    # steps navigate to the listing where a pre-existing org would mask it.
    captured_values.assert_action_succeeded(
        "Create Organization accepted (no backend rejection)",
        error_text=server_response,
        reason="backend rejected the create — organization not created on this run",
        evidence=f"url={page.url!r} server_response={server_response or '<none>'}",
    )
