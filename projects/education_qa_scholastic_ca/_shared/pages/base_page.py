...
from ._selectors import LOCATORS as _BUILTIN_LOCATORS
...
class BasePage:
    ...
    def _read_locators(self) -> dict:
        ...
        try:
            ...
            pass
        return _BUILTIN_LOCATORS

    def selectors(self, page_key: str, key: str) -> list[str]:
        page_map = self._locators.get(page_key) or _BUILTIN_LOCATORS.get(page_key, {})
        value = page_map.get(key)
        if value is None:
            value = _BUILTIN_LOCATORS.get(page_key, {}).get(key, [])
