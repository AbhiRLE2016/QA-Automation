        ...
        for sel in self.selectors(self.PAGE_KEY, "cart_heading"):
            try:
                ...
                if loc.is_visible():
                    ...
                    if "your cart" in text.lower() or "shopping cart" in text.lower() or text.lower() == "cart":
                        return True
            except Exception:
                continue
        return "/cart" in (self.page.url or "")
        ...
        while time.time() < end:
            ...
            for sel in self.selectors(self.PAGE_KEY, "empty_cart_message"):
                try:
                    ...
                    continue
            try:
                line_items = self.page.locator(
                    ", ".join(self.selectors(self.PAGE_KEY, "cart_line_item"))
                ).count()
            except Exception:
                line_items = -1
            try:
