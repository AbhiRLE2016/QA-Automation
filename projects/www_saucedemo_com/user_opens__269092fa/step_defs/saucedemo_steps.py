...
@given(parsers.parse("the user waits for {seconds:d} seconds"))
@when(parsers.parse("the user waits for {seconds:d} seconds"))
@then(parsers.parse("the user waits for {seconds:d} seconds"))
def wait_for_seconds(page: Page, seconds: int) -> None:
...
@when(parsers.parse('the user adds the product "{product_name}" to the cart'))
def add_product_to_cart(page: Page, product_name: str) -> None:
...
def assert_information_page(page: Page, page_title: str) -> None:
    ...
    expected = page_title.strip().lower()
    if "your information" in expected:
        expect(page.locator(".title")).to_contain_text("Your Information")
    else:
