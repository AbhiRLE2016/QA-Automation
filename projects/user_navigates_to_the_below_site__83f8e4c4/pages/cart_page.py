...
class CartPage(BasePage):
    ...
    def subtotal_text(self) -> str:
        """Return the cart subtotal as rendered (e.g. '$48.99'). Falls back to a
        regex scan of the page body if no targeted selector matches."""
        for sel in self.selectors(self.PAGE_KEY, "subtotal_value"):
            try:
                ...
                continue
        body = self.cart_text()
        m = re.search(r"Subtotal[^$]*?\$\s?([\d,]+\.\d{2})", body, re.IGNORECASE)
        if m:
            return f"${m.group(1)}"
        raise AssertionError("Subtotal value not found on cart page")

    def added_to_cart_message_visible(self, timeout: int = 8000) -> bool:
        for sel in self.selectors(self.PAGE_KEY, "added_to_cart_success"):
            try:
                expect(self.page.locator(sel).first).to_be_visible(timeout=timeout)
                return True
            except Exception:
