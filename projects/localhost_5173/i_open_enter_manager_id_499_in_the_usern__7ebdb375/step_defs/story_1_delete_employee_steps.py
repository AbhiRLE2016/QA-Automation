...
def when_user_finds_row(name: str, delete_state: dict, captured_values, test_data) -> None:
    ...
    # Use assert_prerequisite so a missing row produces a clear "BLOCKED"
    # banner in the coverage report — *"could not find Priya Sharma on the
    # Manage Employees table"* — instead of a raw AssertionError.
    # This is the explicit "data is missing, cannot proceed" outcome the
    # user wants when the live app doesn't contain the named employee.
    captured_values.assert_prerequisite(
        f"Locate '{name}' on Manage Employees table",
        condition=pom.employee_row_exists(name),
        reason=(
            f"Employee '{name}' is not present in the Manage Employees "
            "table for manager 499. The story expects this row to exist; "
            "since it does not, the delete cannot proceed. Reporting and "
            "halting per NO-FABRICATION rule (do NOT auto-add the employee)."
        ),
    )
