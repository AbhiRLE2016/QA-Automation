...
class AllEmployeesDirectoryWalkPage(BasePage):
    ...
    # ------------------------------------------------------------------
    # New-story helpers (story_1_verify_all_employees_against_csv)
    # ------------------------------------------------------------------
    def dashboard_is_displayed(self, timeout: int = 15000) -> bool:
        btn = self.first_visible(self.selectors("dashboard", "view_all_employees"), timeout=timeout)
        return btn.is_visible()

    def employees_table_is_displayed(self, timeout: int = 15000) -> bool:
        table = self.first_visible(self.selectors("all_employees", "table"), timeout=timeout)
        return table.is_visible()

    def search_box_is_empty(self) -> bool:
